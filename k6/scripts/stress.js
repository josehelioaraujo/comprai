...
export const options = {
  stages: [{"duration":"1m","target":"Math.round(peakVus * 0.25)"},{"duration":"2m","target":"Math.round(peakVus * 0.50)"},{"duration":"2m","target":"Math.round(peakVus * 0.75)"},{"duration":"2m","target":"peakVus"},{"duration":"1m","target":"Math.round(peakVus * 0.50)"},{"duration":"30s","target":0}],
  thresholds: {"http_req_duration":["p(95)<3000"],"errors":["rate<0.15"]},
  summaryTrendStats: ["avg", "min", "med", "max", "p(90)", "p(95)", "p(99)"],
};

export function handleSummary(data) {
  const dir = __ENV.RESULTS_DIR || "k6/results";
  const structured = buildStructured(data, "stress", "400", "~8.5min", 0.15);
  return {
    [dir + "/summary.json"]: JSON.stringify(data, null, 2),
    [dir + "/stress.json"]:   JSON.stringify(structured, null, 2),
...
export default function () {
  const sid = `stress-vu${__VU}-i${__ITER}`, headers = { "Content-Type": "application/json" };
  ...
  track(res, "search");
  let productId = `mock-${sid}`, productTitle = "Notebook stress", productPrice = 2999.90, productCat = "eletronicos";
  ...
  res = http.post(`${TARGET_URL2}/api/cart/${sid}/items`,
    JSON.stringify({ product: { id: productId, title: productTitle, price: productPrice, category: productCat, source: "k6-stress", imageUrl: null, url: null }, quantity: 1 }),
  ...
  res = http.post(`${TARGET_URL2}/api/checkout/${sid}`,
    JSON.stringify({ name: `stress User VU${__VU}`, email: `stress-vu${__VU}@k6.test`, phone: "11999999999", address: "Rua stress, 1, Sao Paulo, SP" }),
