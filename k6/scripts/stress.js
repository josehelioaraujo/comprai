...
export default function () {
  ...
  track(res, "search");
  let productId = `mock-${sid}`; let productTitle = "Produto Stress"; let productPrice = 1999.90; let productCat = "eletronicos";
  if (res.status === 200) {
    try {
  ...
  res = http.post(`${TARGET_URL}/api/checkout/${sid}`,
    JSON.stringify({ name: `Stress User VU${__VU}`, email: `stress-vu${__VU}@k6.test`, phone: "11977776666", address: "Rua Stress, 400, São Paulo, SP" }),
    { headers });
  track(res, "checkout");
  let orderId = null;
