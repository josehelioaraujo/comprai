...
public class GetCartQueryHandlerTests
{
    ...
    private static CartItemDto MakeItem(string id, decimal price, int qty) =>
        new(id, new ProductDto(id, "Produto", price, null, null, null, "mock"), qty, price * qty);
    ...
    [Fact]
    public async Task Handle_RetornaExatamenteOQueVeioDoPort()
    {
        var sessionId = _faker.Random.Guid().ToString();
        var expected = new List<CartItemDto>
        {
            MakeItem("item-1", 100m, 2),
            MakeItem("item-2", 50m,  1),
        };
        _cart.GetItemsAsync(sessionId, Arg.Any<CancellationToken>()).Returns(expected);
        var handler = new GetCartQueryHandler(_cart);
    ...
    [Fact]
    public async Task Handle_IsSuccessTrue_Sempre()
    {
        var sessionId = _faker.Random.Guid().ToString();
        _cart.GetItemsAsync(sessionId, Arg.Any<CancellationToken>()).Returns(new List<CartItemDto>());
        var handler = new GetCartQueryHandler(_cart);
