...
public static class ObservabilityEndpoints
{
    public static void MapObservabilityEndpoints(this WebApplication app)
    {
            ...
            CancellationToken ct) =>
        {
            ...
            var search   = await QueryInstant(client, baseUrl, "sum(ucp_search_requests_total)",    ct);
            var cart     = await QueryInstant(client, baseUrl, "sum(ucp_cart_add_items_total)",   ct);
            var checkout = await QueryInstant(client, baseUrl, "sum(ucp_checkout_success_requests_total)", ct);
            var orders   = await QueryInstant(client, baseUrl, "sum(ucp_order_placed_orders_total)", ct);
            var chkFail  = await QueryInstant(client, baseUrl, "sum(ucp_checkout_requests_total)", ct);
            ...
            CancellationToken ct) =>
        {
            ...
            var hits   = await QueryInstant(client, baseUrl, "sum(ucp_cache_hit_hits_total)",  ct);
            var misses = await QueryInstant(client, baseUrl, "sum(ucp_cache_miss_misses_total)", ct);
            ...
            CancellationToken ct) =>
        {
            ...
            var totalIntents  = await QueryInstant(client, baseUrl, "sum(ucp_intent_detected_requests_total)", ct);
            var ollamaCalls   = await QueryInstant(client, baseUrl, "sum(ucp_ollama_request_requests_total)",  ct);
            var ollamaErrors  = await QueryInstant(client, baseUrl, "sum(ucp_ollama_error_errors_total)",    ct);
    ...
    // ── Helpers Prometheus ────────────────────────────────────────────────────
        ...
        app.MapGet("/api/observability/logs/access", async (
            string? method, string? status, string? path, int? limit,
            IConfiguration config, IHttpClientFactory factory, CancellationToken ct) =>
        {
            var baseUrl = config["Observability:LokiUrl"] ?? "http://comprai-loki:3100";
            var n       = limit ?? 200;
